...
CREATE TABLE IF NOT EXISTS `fitness_club_db`.`users_photo` (
  ...
  `image_url` VARCHAR(200) NULL DEFAULT 'https://i.postimg.cc/Wbznd0qn/1674365371-3-34.jpg',
...
CREATE TABLE IF NOT EXISTS `fitness_club_db`.`trainers_accounts` (
  ...
  INDEX `fk_trainers_accounts_trainers_photo1_idx` (`id_photo` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_trainers_accounts_trainers_photo1`
    FOREIGN KEY (`id_photo`)
    REFERENCES `fitness_club_db`.`trainers_photo` (`id_photo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
